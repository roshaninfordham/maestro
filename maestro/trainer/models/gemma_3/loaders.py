...
def train_collate_fn(
    batch: list[tuple[Image.Image, dict[str, Any]]], processor: Gemma3Processor, max_length: int = 512
):
...
def evaluation_collate_fn(batch: list[tuple[Image.Image, dict[str, Any]]], processor: Gemma3Processor):
    images, data = zip(*batch)
    prefixes = ["<image>" + entry["prefix"] for entry in data]
    suffixes = [entry["suffix"] for entry in data]
